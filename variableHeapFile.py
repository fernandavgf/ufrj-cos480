...
def createHeapBD(csvFilePath):
    ...
    valuesToLoad = aux.padRecords(aux.readFromFile(csvFilePath), variablePadding=True)
...
def HeapSelectRecord(colName, value, singleRecordSelection = False, valueIsArray = False, secondColName = "", secondValue = "", betweenTwoValues=False):
    ...
    while not (recordFound or endOfFile):
        currentBlock = aux.fetchBlock(dbPath, currentRecord, variableHeap=True)#pega 5 registros a partir do registro atual
        if currentBlock == []:
            endOfFile = True
            break
...
def HeapInsertSingleRecord(listOfValues, checkPrimaryKey=True, showSuccessMsg=True, addPaddingCharacter=False):
    global blankLines
    ...
    if addPaddingCharacter:
        listOfValues = [value + aux.paddingCharacter for value in listOfValues]

    if blankLines:
        row = ''.join(listOfValues)[:-1]
        # row = aux.fillCod(listOfValues[0])
        # for i in range(1, len(listOfValues)):
        #     row += aux.padString(listOfValues[i], aux.maxColSizesList[i], variablePadding=True)
        ...
        with open(dbPath, 'a') as file:
            ...
            for i in range(len(listOfValues)):
                if i == len(listOfValues) - 1: file.write(listOfValues[i][:-1])
                else: file.write(listOfValues[i])

            # file.write(listOfValues[0])
            # #assumindo que estão na ordem correta já
            # for i in range(1, len(listOfValues)):
            #     file.write(aux.padString(listOfValues[i], aux.maxColSizesList[i], variablePadding=True))
...
def HeapInsertMultipleRecord(listOfRecords, checkPrimaryKey=True):
    for record in listOfRecords:
        HeapInsertSingleRecord(record, checkPrimaryKey, addPaddingCharacter=True)
